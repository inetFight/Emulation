		...
		SendButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(barcode1.getText().isEmpty() 
					...
					JOptionPane.showMessageDialog(frm, "Введите хоть один штрихкод", "Error", JOptionPane.ERROR_MESSAGE);
				}
				else {
					barcodes.clear();
					if(!barcode1.getText().isEmpty()){barcodes.add(barcode1.getText());}
					if(!barcode2.getText().isEmpty()){barcodes.add(barcode2.getText());}
					if(!barcode3.getText().isEmpty()){barcodes.add(barcode3.getText());}
					if(!barcode4.getText().isEmpty()){barcodes.add(barcode4.getText());}
					if(!barcode5.getText().isEmpty()){barcodes.add(barcode5.getText());}
					try {
